            ...
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                if handedness.classification[0].label == "Right":
                    ...
                    if (prev_wrists != None) and (hand_sign_id == 1):
                        ...
                        sensetivity = 3
                        mouse.move(int((speed[0])/cap_width*WIDTH), int((speed[1])/cap_height*HEIGHT))
                    prev_wrists = wrist
                if handedness.classification[0].label == "Right" and hand_sign_id == 2:
                    direction = calc_direction(landmark_list[8], landmark_list[5])
                    if direction == "left":
                        if click_left == False:
                            mouse.holdclick(mouse.left)
                        click_left = True
                    if direction == "right":
                        if click_right == False:
                            mouse.holdclick(mouse.right)
                        click_right = True
                    if direction != "left":
                        click_left = False
                        mouse.releaseclick(mouse.left)
                    if direction != "right":
                        click_right = False
                        mouse.releaseclick(mouse.right)
